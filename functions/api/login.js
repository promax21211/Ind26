...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const user = await env.DB.prepare(
      'SELECT email, name, nickname, password_hash, promo_code, discord_status, created_at FROM users WHERE email = ?'
    )
      .bind(email)
      .first()
    ...
    return json({ error: 'Something went wrong signing you in. Please try again.' }, { status: 500 })
  }
}
